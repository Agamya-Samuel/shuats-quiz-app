...
interface FileUploadProps {
  ...
  onUploadSuccess: (status: { uploaded: boolean }) => void
}
...
export function FileUpload({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      const isValidType = validTypes.some((type) => fileType.includes(type.toLowerCase()))
  ...
  const renderUploadState = () => {
    if (status?.verified) {
      return (
        <div className="flex flex-col items-center justify-center p-6 text-center">
          ...
          <p className="text-sm text-green-600 mt-1">This document has been verified</p>
          <Button
            variant="outline"
            className="mt-4"
            onClick={() => {
    ...
    if (status?.rejected) {
      return (
        <div className="flex flex-col items-center justify-center p-6 text-center">
          ...
          <p className="text-sm text-red-600 mt-1">{status.rejectionReason || "Please upload a new document"}</p>
          <Button
            variant="outline"
            className="mt-4"
            onClick={() => {
    ...
    if (status?.uploaded) {
      return (
        <div className="flex flex-col items-center justify-center p-6 text-center">
          ...
          <p className="text-sm text-amber-600 mt-1">Your document is being reviewed</p>
          <Button
            variant="outline"
            className="mt-4"
            onClick={() => {
    ...
    return (
      ...
      >
        ...
        <h3 className="font-medium text-center">Click to upload {title}</h3>
    ...
    <Card className="overflow-hidden">
      <CardHeader className="bg-gradient-to-r from-teal-50 to-emerald-50">
        <div className="flex items-center justify-between">
          ...
          {status?.verified && <Badge className="bg-green-500">Verified</Badge>}
